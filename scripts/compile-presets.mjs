 ...
 *
 * WHITE LABEL. The compiled artifacts carry no trace of the registry a theme
 * was ingested from: no author handle, no identifier, no URL. Provenance is
 * kept in `_catalog.json`, which this script reads from disk and the app
 * never imports — so attribution survives in the repository while the shipped
 * bundle is entirely Scaffold's. `normalizeDesignDoc()` enforces the same rule
 * on the raw DESIGN.md files, which Raw mode renders verbatim.
 *
...
/** Names of the external registry, in the forms the corpus actually uses. */
const REGISTRY_MARKS = [
  // The bracketed notice the registry injects at the top of a served file.
  /^\s*\[DESIGNmd:[^\]]*\]\s*\n+/i,
]
...
/**
 * Read a DESIGN.md and strip every trace of the registry it came from.
 *
 * Two different things get removed, and they are not the same kind of thing:
 *
 *   1. The bracketed banner the registry injects into every file it serves.
 *      That is not the author's content at all — it is envelope, and removing
 *      it changes nothing about the design system.
 *   2. The bare domain where an author happened to name it in prose ("…and
 *      designmd.ai style workflows"). That IS the author's sentence, so the
 *      domain is swapped for `Scaffold` rather than the clause being deleted:
 *      the sentence keeps its meaning and stops advertising.
 *
 * Rewritten in place and idempotent, so re-ingesting a theme and re-running
 * the compiler leaves the catalog white-label without anyone remembering to
 * do it by hand.
 *
 * Provenance is NOT destroyed — it stays in `_catalog.json`, which the app
 * never imports.
 */
function normalizeDesignDoc(slug) {
  const path = join(PRESETS, slug, 'DESIGN.md')
  const original = readFileSync(path, 'utf8')

  let md = original
  for (const mark of REGISTRY_MARKS) md = md.replace(mark, '')
  md = md.replace(/\bdesignmd\.ai\b/gi, 'Scaffold').replace(/\bDESIGNmd\b/g, 'Scaffold')
...
function compile(slug, source) {
  const md = normalizeDesignDoc(slug)
  ...
  const tokens = {
    ...
    // No `source` block. Provenance lives in `_catalog.json`, which the
    // compiler reads from disk and the app never imports — so attribution is
    // retained in the repository while nothing about an external registry
    // reaches the shipped bundle. See the white-label note at the top.
...
const registry = rows.map(({ slug }) => {
  ...
  return {
    id: t.id,
    name: t.name,
    // Deliberately no author / identifier / url: the registry is imported by
    // the app and everything in it ships in the bundle. Provenance stays in
    // `_catalog.json`, which only this script reads.
...
console.log(
  normalizedCount
    ? `${normalizedCount} DESIGN.md file(s) normalized to white label`
    : 'all DESIGN.md files already white label',
)
